  ...
  object LockService {
    ...
    def create(id: LockKey, data: IzanamiLock): ZIO[LockContext, IzanamiErrors, IzanamiLock] =
      for {
        ...
        mayBeFeature        <- FeatureDataStore.>.getById(id.drop(LockType.FEATURE)).orDie
        mayBeSubFeature     <- FeatureDataStore.>.findByQuery(Query.oneOf(List(s"${id.drop(LockType.FEATURE).key}:*")), 1, 1).orDie
        -                   <- IO.when(mayBeFeature.isEmpty && mayBeSubFeature.results.isEmpty)(IO.fail(DataShouldExists(id.drop(LockType.FEATURE)).toErrors))
